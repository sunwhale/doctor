...
The mass flow of the cooling water has to be less than 1.5 l/min. Otherwise, the strain control loop will be instable.
...
\noindent
The monotonic tensile test is the fundamental material test in which a specimen is subjected to a controlled elongation until failure.
...
\begin{figure}[!htp]
\centering{\includegraphics[width=13cm]{Furnace653_1.jpg}}
...
After that only the actuator displacement and the force were recorded. The fracture elongation was calculated from the actuator displacement.
...
The fundamental material properties of Inconel 718 at different temperatures can be determined from the experimental data, such as Young's modulus, yield strength, ultimate tensile stress and fracture elongation (see Table \ref{tab:General_material_mechanical_properties}).
...
Axial stress $\sigma$, refers to the engineering stress, which is the ratio of force, $F$, to specimen original cross-sectional area, $A_0$, that is, $\sigma=F/A_0$.
...
For convenience, hereafter, the six strain paths which are shown in \ref{Fig:LoadPath} from (a) to (f) are denoted as 'TC', 'TOR', 'CRO', 'PRO', 'NPR' and 'CIR', respectively.
\begin{figure}[ht]
...
Loading ratio $R_\varepsilon$ means the ratio of minimum mechanical strain to the maximum mechanical strain in a loading cycle. Under the axial-torsional condition, the loading ratio is calculated from the axial strains.
...
Failure was deemed to have occurred if the peak load dropped 15\% from its stabilized value. A 30\% load drop in comparing with the stabilized peak value was defined as failure.
...
\begin{figure}[!htp]
  ...
  \put(84,13){(d)}
  \end{overpic}
  \caption{Experimental results of CIR-IF tests at temperatures 300$^\circ$C, 550$^\circ$C, and 650$^\circ$C: (a) axial stress-strain loops, (b) axial peak-valley stresses, (c) torsional stress-strain loops, (d) torsional peak-valley stresses.}
...
\begin{itemize}
  ...
  \begin{equation}
  \varepsilon_{\rm{th}}=\alpha\left(T\right)\Delta T.
  \label{Equ:thermal_strain}
  \end{equation}
  The thermal strain is generally isotropic.
  \item {\em Mechanical strain}, $\varepsilon_{\rm{mech}}$, is resulting from applied load and directly related with material deformation and damage. Under tension-torsion loading conditions, the mechanical deformation in testing is characterized by the maximum axial strain.
  ...
  R_{\varepsilon}=\varepsilon_{\rm{mech,min}}/\varepsilon_{\rm{mech,max}}.
  \end{equation}
  Under axial-torsional loading condition, the mechanical strains are the axial strains.
  \item {\em Phase angle of the thermal loading and mechanical loading}, $\theta_{T-\varepsilon}$, represents the phase difference between maximum values of temperature and mechanical strain.
  \item {\em Phase angle of the thermal loading and torsional loading}, $\theta_{T-\gamma}$, represents the phase difference between maximum values of temperature and shear strain.
  \item {\em Phase angle of the mechanical loading and torsional loading}, $\theta_{\varepsilon-\gamma}$, represents the phase difference between maximum values of mechanical strain and shear strain.
  \item {\em Equivalent mechanical strain}, $\varepsilon_{\rm{eq}}$, is calculated by the von Mises relation\cite{Pol1991Cyclic},
  \begin{equation}
  \varepsilon_{\rm{eq}}\left(t\right)=\sqrt {\varepsilon_{\rm{mech}}^2\left(t\right) + {\gamma ^2}\left(t\right)/3}.
  \end{equation}
\end{itemize}
