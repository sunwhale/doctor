\chapter{Low cycle fatigue assessment of isothermal and thermomechanical fatigue}
...
% Brown和Miller准则由一系列由最大剪应变${\gamma _{\max }}$和最大剪应变平面上的法向应变${\varepsilon _{\rm{n}}}$为坐标所组成的$\Gamma$平面上的等寿命曲线组成，对于给定寿命有
% \[{\gamma _{\max }} = f\left( {{\varepsilon _{\rm{n}}}} \right)\]
...
% \[\frac{{\Delta {\gamma _{\max }}}}{2} + k\Delta {\varepsilon _{\rm{n}}} = C\]
% 其中，$\Delta {\varepsilon _{\rm{n}}}$是临界面上的法向应变变幅，$k$和$C$是材料常数。

% % Wang和Brown[37]用相邻两个最大剪应变折返点之间的法向应变变程${\varepsilon _{\rm{n}}^*}$来代替法向应变变程$\Delta {\varepsilon _{\rm{n}}}$，给出疲劳破坏模型
% % \[\frac{{\Delta {\gamma _{\max }}}}{2} + k\varepsilon _{\rm{n}}^* = C\]
% % 其中$k$和$C$是材料常数。
...
% 代替法向应变变程$\Delta {\varepsilon _{\rm{n}}}$作为参数，提出准则如下：
% \begin{equation}
...
% \begin{eqnarray}
% {\left( {\Delta {\sigma _n}\Delta {\varepsilon _{\rm{n}}}} \right)_{\max }} + \left( {\Delta \tau \Delta \gamma } \right) &=& \frac{{4{{\sigma '}_f}^2}}{E}{\left( {2{N_f}} \right)^{2b}} + 4{{\sigma '}_f}{{\varepsilon '}_f}{\left( {2{N_f}} \right)^{b + c}}
% \end{eqnarray}

% \begin{eqnarray}
% \left( {\Delta {\sigma _n}\Delta {\varepsilon _{\rm{n}}}} \right) + {\left( {\Delta \tau \Delta \gamma } \right)_{\max }} &=& \frac{{4{{\tau '}_f}^2}}{G}{\left( {2{N_f}} \right)^{2b\gamma }} + 4{{\tau '}_f}{{\gamma '}_f}{\left( {2{N_f}} \right)^{b\gamma  + c\gamma }}
% \end{eqnarray}
...
% In this chapter, the isothermal and thermomechanical fatigue life are investigated experimentally and computationally.
...
\begin{table*}[htbp]
  \centering
  \caption{Experimental conditions and results of isothermal (650$^\circ$C) and thermomechanical (300-650$^\circ$C) fatigue tests.} \vspace{0.1cm}
    \begin{tabular}{p{2cm}p{1.2cm}p{1.2cm}p{1.2cm}p{2.5cm}p{1cm}p{1cm}p{1cm}p{1cm}}
    \hline
    Test Type & $\pm \varepsilon _{\rm mech}$ & $\pm \gamma/ \sqrt 3$ & $\varepsilon _{\rm eq}$ & $\dot \varepsilon _{\rm eq}$ & $\theta_{T-\varepsilon}$ & $\theta_{T-\gamma}$ & $\theta_{\varepsilon-\gamma}$ & $N_{\rm f}$ \\
...
\begin{table}[htbp]
  ...
  \caption{Basic properties of Nickel-based superalloy Inconel 718 at 650$^{\circ}$C.}
    ...
    \hline
          & $E$     & $K'$     & $n'$     & $\sigma'_{\rm f}$    & $b$     & $\varepsilon'_{\rm f}$    & $c$ \\
    \hline
...
\begin{figure}[!htp]
  \centering
  \includegraphics[width=14cm]{plot_exp_half_life_cycle-TC-IF.pdf}
  \caption{Comparison of the stress-strain hysteresis loops of the half life cycle $N_{\rm f}/2$ between experiments and Ramberg-Osgood model, with strain ranges of $\pm0.4\%, \pm0.6\%, \pm0.8\%$ and $\pm1.0\%$ at 650$^{\circ}$C.}
  \label{Fig:plot_exp_half_life_cycle}
\end{figure}

\begin{figure}[!htp]
  \centering
  \includegraphics[width=14cm]{plot_monotonic_cyclic_osgood.pdf}
  \caption{Monotonic and cyclic stress-strain curves with Ramberg-Osgood relationship for Inconel 718 at 650$^{\circ}$C.}
  \label{Fig:plot_monotonic_cyclic_osgood}
\end{figure}

\begin{figure}[!htp]
  \centering{\includegraphics[width=14cm]{plot_exp_coffin_manson.pdf}}
  \caption{Manson-Coffin plots of the isothermal fatigue tests at 650$^{\circ}$C.}
  \label{Fig:Baseline}
\end{figure}

\ref{Fig:Baseline} shows comparison of experimental results by NASA \cite{kim1988elevated,nelson1992creep} and BHU \cite{Mahobia2014} as well as the data from the present work. The partially significant difference is observed in the three series of testing, which may be induced by different material providers. Heat treatments and processing of all the three series of specimens are similar. From this observation, one may expect obvious deviations in fatigue performance for the same nominal superalloy Inconel 718.
...
\end{equation}
where ${{{\sigma '}_{\rm f}}}$ is the fatigue strength coefficient, $b$ is the fatigue strength exponent, ${{{\varepsilon '}_{\rm f}}}$ is the fatigue ductility coefficient, and $c$ is the fatigue ductility exponent. The model parameters can be determined from uniaxial fatigue data. The results are summarized in Table \ref{tab:MechanicalProperties} for all three series of experiments.
...
At the mechanical strain amplitude bigger than 0.6\%, the in-phase thermomechanical fatigue tends to have a lower fatigue lifetime. 
% Oppositely, the in-phase thermomechanical fatigue lifetimes are longer than the out-of-phase at the mechanical strain amplitude smaller than 0.6\%.

% \ref{Fig:plot_monotonic_cyclic_osgood} shows the Mises equivalent strain amplitude vs. number of cycles to failure under four loading conditions: uniaxial in phase IP TMF, uniaxial out of phase OP TMF, proportional axial-torsional IP TMF and non-proportional axial torsional (diamond path) IP TMF.
...
\begin{figure}[!htp]
  \centering{\includegraphics[width=14cm]{plot_exp_fatigue_life_tmf.pdf}}
...
\begin{figure*}
  ...
  \nonumber
    ...
    \begin{overpic}[width=8.0cm]{7112-4.jpg}
      ...
      \put(66,50){\color{white}\thicklines\circle{20}}
      \put(55,35){\color{white}Fatigue Striations}
  ...
  \nonumber
    ...
    \begin{overpic}[width=8.0cm]{7033-102.jpg}
      ...
      \put(55,35){\color{white}\thicklines\circle{20}}
      \put(60,20){\color{white}Fatigue Striations}
    \end{overpic}
  \end{minipage}%
  \begin{minipage}[t]{0.5\linewidth}
    \centering
...
\end{equation}
where ${{\Delta \hat \gamma }}/{2}$ is the equivalent shear strain amplitude \cite{Wang1993}. $\Delta {\varepsilon _{\rm n}}$ represents the normal strain acting on the plane with the maximum shear strain range $\Delta {\gamma _{\max}}$. The material dependent parameter $S$ represents the influence of the normal strain on the crack propagation.
...
\label{Equ:FS_model}
\end{equation}
where ${{\tau '}_{\rm f}}$ is the torsional fatigue strength coefficient, ${b_0}$ is the torsional fatigue strength exponent, ${{\gamma '}_{\rm f}}$ is the torsional fatigue ductility coefficient and ${c_0}$ is the torsional fatigue ductility exponent. Approximations were suggested as ${{\tau '}_{\rm f}}\approx {{\sigma '}_{\rm f}}/\sqrt{3}$, ${{\gamma '}_{\rm f}}\approx \sqrt{3} {{\sigma '}_{\rm f}}$, ${b_0}\approx b$, ${c_0}\approx c$.
...
\caption{Rotations of the Cartesian coordinate system.}
...
The second step is rotation along the $x'_2$-axis. As shown in \ref{Fig:coordinate_transformation_of_tensor}, if we define the angle from $x'_3$-axis to $x''_1$-axis as $\theta$, the rotation angle along the $x'_2$-axis is $\theta-{\pi}/{2}$. The transformation matrix from ($x'_1,x'_2,x'_3$) to ($x''_1,x''_2,x''_3$) is given by direction cosines as:
\begin{eqnarray}
\begin{array}{*{20}{l}}
...
\end{equation}
Contrary to the normal stress, the shear stress ${{\tau _{{{n}}}}(t)}$ relative to the material plane $\Gamma$ is a vector. It varies in both direction and magnitude during the loading cycle.
...
Except the Zamrik's model, the left-hand sides of the fatigue models in Equations (\ref{Equ:BM_model}), (\ref{Equ:FS_model}), (\ref{Equ:SWT_model}), (\ref{Equ:Liu1_model}) and (\ref{Equ:CCB_model}) which are also called the fatigue damage parameters, are given by:
\begin{eqnarray*}
...
where the constant $k$ is obtained by Equation (\ref{Equ:k_in_FS_model}) as 0.214, and ${\sigma _{\rm y}}$ is the yield stress which can be found in Table \ref{tab:General_material_mechanical_properties}.
...
\section{Thermomechanical fatigue life assessment}
\noindent
...
Based on such considerations, Equation (\ref{Equ:LiuModelwithStressRatio}) is modified into
...
As discussed by V\"{o}se \cite{Vose2013}, the correction term A is defined as
\begin{equation}
...
As given in the Ref. \cite{Warren2008}, the activation energy $Q_0$ is set to the value of 240 kJ/mol and the intrinsic activation volume $\upsilon _0^*$ is used the value of $3.51\times10^{-4}$ $\rm{m}^{3}/\rm{mol}$. The ultimate strength $\sigma_{\rm{ult}}$ of Inconel 718 at 650$^\circ$C was measured under monotonic tensile tests as $1305$ MPa. The constants $k$, $B$ and $C$ are determined by the optimization method to minimize the life prediction mean-squared error. Finally, $k$ is set to 1.2, $B$ is set to 0.49 and $C$ is set to $1.29\times10^{12}$ $\rm{s}^{-1}$.
...
\ref{fig:PresentModel} shows the life assessment of the multiaxial TMF tests based on the suggested life model (Equation (\ref{Equ:TMF_model})). In \ref{fig:PresentModel}(a), the fatigue damage parameter versus experimental fatigue lifetime is displayed. The fatigue damage parameter shows a good correlation with the experimental fatigue lifetime under uniaxial TMF loading case as well as proportional and non-proportional TMF loading conditions. In \ref{fig:PresentModel}(b), one can observe that, based on the proposed model, most of the predicted fatigue lifetimes are within the scatter band with a factor of 2. It shows a good agreement with the experimental and predicted fatigue lifetimes.
...
\end{overpic}
\caption{The quantitative evaluation of the TMF life prediction results: (a) $T_{\rm{N}}$, (b) $T_{\rm{RMS}}$.}
\label{Fig:plot_fatigue_life_quantitative_evaluation_tmf}
\end{figure}

\section{Summary}
\noindent
% This chapter has described the experimental facility of non-proportional thermomechanical fatigue tests. 
