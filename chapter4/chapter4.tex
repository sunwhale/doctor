...
\noindent
This chapter focuses on the multiaxial cyclic behavior and the stress relaxations of Inconel 718 at varying temperatures between 300$^\circ$C and 650$^\circ$C. Effects of non-proportional cyclic loading as well as thermomechanical coupling are studied. 
The superalloy Inconel 718 is assumed to be rate-independent in the present work. In the turbine industry, the material is used up to a temperature of 650$^\circ$C for both rotating and casing parts. Extensive experiments \cite{kim1988elevated, Schlesinger2017} showed that the stress-strain responses upon 650$^\circ$C were hardly influenced by the strain rate. In the turbine industry, both rotating and casing parts of Inconel 718 are computed by using the rate-independent plasticity. For low cycle fatigue, the strain rate is low. For fatigue life assessment only the stabilized strains and stresses are needed. The rate-dependence is meaningful for creep fatigue in this case, but not for LCF. It is a common practice to use the rate-independent plasticity in fatigue life prediction. The rate-independent plasticity can be directly used for thermomechanical fatigue assessment. On the other side, the fundamentals for the rate-dependent model are not so different from the rate-independent ones. However, experiments reveal the complex constitutive behavior of the material, such as cyclic hardening/softening, non-proportional hardening, thermomechanical phase effect, non-Masing effect, etc. Quantitative description of the material behavior needs a constitutive model based on the extensive experimental investigation. In the present work, a modified cyclic plasticity model is suggested for the Nickel-based superalloy Inconel 718 under multiaxial thermomechanical cyclic loading conditions and can give a uniform description of the material modeling for both isothermal and thermomechanical fatigue loading components.
...
\section{Fundamentals of continuum plasticity}
\noindent
...
In the section, the fundamentals of time-independent phenomenological plasticity are introduced, such as yield surface, flow rule, Drucker postulates, consistency condition, isotropic and kinematic hardening.
...
% In the elastic region, the stress is proportional to the strain if the stress in the specimen is below a certain value, i.e., the yield stress $\sigma_{\rm y0}$, which is the elastic limit.
...
% For strain hardening materials the yield stress increases with increasing plastic deformation to a value of $\sigma_{\rm y}$.
...
% The recovered strain, $\varepsilon_{\rm e}$, is the elastic strain and it can be seen that the total strain, $\varepsilon$, is the sum of the two
% \begin{equation}
...
\begin{equation}
F\left( {{\sigma _{ij}}}, Y \right) = 0,
\label{Equ:yield_function_1}
\end{equation}
where $F\left( {{\sigma _{ij}}} \right)$ is a function of the stress tensor and $Y$ is a critical value. Generally, Equation (\ref{Equ:yield_function_1}) is called yield function. In the principal stress space, the Equation (\ref{Equ:yield_function_1}) can be expressed as
\begin{equation}
F\left( {{\sigma _1},{\sigma _2},{\sigma _3}}, Y \right) = 0,
\end{equation}
...
\begin{equation}
\frac{1}{2}\max \left( {\left| {{\sigma _1} - {\sigma _2}} \right|,\left| {{\sigma _2} - {\sigma _3}} \right|,\left| {{\sigma _3} - {\sigma _1}} \right|} \right) = \tau_{\rm y},
\end{equation}
where $\tau_{\rm y}$ is the yield strength in shear.
...
\begin{equation}
{\left( {{\sigma _1} - {\sigma _2}} \right)^2} + {\left( {{\sigma _2} - {\sigma _3}} \right)^2} + {\left( {{\sigma _3} - {\sigma _1}} \right)^2} = 2\sigma _{\rm y}^2,
\end{equation}
where $\sigma_{\rm y}$ is the yield strength in tension.
...
In material models, in which isotropic hardening is depicted, it is done by evolution equations describing an increase in the radius $k$ of the yield surface. In such cases, the yield surface is expanded as far as it finally only purely elastic state occurs.
...
\ref{Fig:isotropic_hardening} and \ref{Fig:kinematic_hardening} show the schematic diagram of isotropic and kinematic hardening, respectively.
...
\section{A review of cyclic plasticity theories}
...
\end{equation}
where ${\bf{a}}$ is the total back stress, ${{{\bf{a}}^k}}$ is a part of the total back stress, $k = 1\sim M$ , and $M$ is the number of back stress parts considered. Each back stress, ${{{\bf{a}}^k}}$ follows an A-F type relation:
\begin{equation}
{{\dot{\bf a}}^k} = \frac{2}{3}{c^k}{{\bm{\dot \upepsilon }}^{\rm{p}}} - {\gamma ^k}{{\bf{a}}^k}\dot p.
\end{equation}
...
\begin{equation}
{\dot{\bf a}}^k = \frac{2}{3}c^k{{\bm{\dot \upepsilon }}^{\rm{p}}} - {\gamma ^k} \left\langle {1 - \frac{{\bf{a}}^k}{{\left\| {\bf{a}}^k \right\|}}} \right\rangle {\bf{a}}^k\dot p,
\end{equation}
where
\begin{equation}
\left\| {\bf{a}}^k \right\| = \sqrt {{\bf{a}}^k:{\bf{a}}^k}.
\end{equation}
...
\begin{equation}
{\dot{\bf a}} = \sum\limits_{k = 1}^M {{{{\dot{\bf a}}}^k}},
\end{equation}
For each back stress, ${\bf{a}}^k$, it takes two forms as following.

\textbf{Model I}, the extreme case, corresponding to some multilinear model.
The decomposition of the kinematic hardening variable into components is based on the assumption that each component has a critical state for its dynamic recovery to be activated fully.
The critical state is introduced by a surface ${f^k} = \left\| {{{\bf{a}}^k}} \right\| - {c^k}/{\gamma ^k} = 0$.
...
\begin{equation}
{{\dot{\bf a}}^k} = \frac{2}{3}{c^k}{{\bm{\dot \upepsilon }}^{\rm{p}}} - {\gamma ^k}H\left( {{f^k}} \right)\left\langle {{\bf{n}}:{{\bf{L}}^k}} \right\rangle {{\bf{a}}^k}\dot p,
\end{equation}
in which
\begin{equation}
{{\bf{L}}^k} = \frac{{{{\bf{a}}^k}}}{{\left\| {{{\bf{a}}^k}} \right\|}},
\end{equation}
or
\begin{equation}
{{\dot{\bf a}}^k} = {\zeta ^k}\left[ {\frac{2}{3}{r^k}{{{\bm{\dot \upepsilon }}}^{\rm{p}}} - H\left( {{f^k}} \right)\left\langle {{{{\bm{\dot \upepsilon }}}^{\rm{p}}}:\frac{{{{\bf{a}}^k}}}{{{{\bar a}^k}}}} \right\rangle {{\bf{a}}^k}} \right],
\label{Eqn:Introduction2}
\end{equation}
where $H$ is the Heaviside step function, ${\bf{n}}$ the plastic strain rate direction.
The Macaulay bracket introduces the consistency condition for the critical state ${f^k} = {\dot f^k} = 0$.
...
\begin{equation}
{{\dot{\bf a}}^k} = \frac{2}{3}{c^k}{{\bm{\dot \upepsilon }}^{\rm{p}}} - {\gamma ^k}{\left( {\frac{{\left\| {{{\bf{a}}^k}} \right\|}}{{{c^k}/{\gamma ^k}}}} \right)^{{m^k}}}\left\langle {{\bf{n}}:{{\bf{L}}^k}} \right\rangle {{\bf{a}}^k}\dot p,
\end{equation}
or
\begin{equation}
{{\dot{\bf a}}^k} = {\zeta ^k}\left[ {\frac{2}{3}{r^k}{{{\bm{\dot \upepsilon }}}^{\rm{p}}} - {{\left( {\frac{{{{\bar a}^k}}}{r}} \right)}^{{m^k}}}\left\langle {{{{\bm{\dot \upepsilon }}}^{\rm{p}}}:\frac{{{{\bf{a}}^k}}}{{{{\bar a}^k}}}} \right\rangle {{\bf{a}}^k}} \right].
\end{equation}
Though not necessary, the Macaulay bracket $\left\langle {{\bf{n}}:{{\bf{L}}^k}} \right\rangle$ continues to operate in this smooth version, and will play a significant role in reducing ratchetting.
...
In the first version of the Ohno and Wang's model, the dynamic recovery of ${{\bf{a}}^k}$ is assumed to take place only in a critical state, which is taken to be a hyper sphere of radius $r^k$ in the space of ${{\bf{a}}^k}$ such as
\begin{equation}
{f^k} = \frac{3}{2}{{\bf{a}}^k}:{{\bf{a}}^k} - \left( r^k \right)^2 = 0.
\end{equation}
Let us assume the two kinds of dynamic recovery terms mentioned above. Then, with Heaviside's step function and Macaulay bracket, the evolution equation of ${{\bf{a}}^k}$ can be expressed as
\begin{equation}
{{\dot{\bf a}}^k} = {\zeta ^k}\left[ {\frac{2}{3}{r^k}{{{\bm{\dot \upepsilon }}}^{\rm{p}}} - {\mu ^k}{{\bf{a}}^k}\dot p - H\left( {{f^k}} \right)\left\langle {{{\dot \lambda }^k}} \right\rangle {{\bf{a}}^k}} \right],
\label{Equ:Introduction1}
\end{equation}
where $\zeta^k$ and $\mu^k$ are material parameters, and ${\dot \lambda }^k$ is determined to have the following form using the consistency condition ${\dot f^k} = 0$:
\begin{equation}
{\dot \lambda ^k} = {\bm{\dot \upepsilon }}^{\rm{p}}:\frac{{{{\bf{a}}^k}}}{{{r^k}}} - {\mu ^k}\dot p.
\end{equation}
The second and third terms in the right hand side in Equation (\ref{Equ:Introduction1}) express the dynamic recovery of ${\bf{a}}^k$ based on the Armstrong and Frederick model and the Ohno and Wang model, respectively, whereas the first term is responsible for strain hardening.
\begin{equation}
{{\dot{\bf a}}^k} = {\zeta ^k}\left( {\frac{2}{3}{r^k}{{{\bm{\dot \upepsilon }}}^{\rm{p}}} - \left[ {{\mu ^k} + H\left( {{f^k}} \right)\left( {1 - {\mu ^k}} \right)} \right]\left\langle {{{{\bm{\dot \upepsilon }}}^{\rm{p}}}:\frac{{{{\bf{a}}^k}}}{{{r^k}}}} \right\rangle {{\bf{a}}^k}} \right),
\end{equation}
where $0 \le {\mu ^k} \le 1$ Critical examination for this rule indicates that continuous ratchetting with constant rate occurs as long as ${\mu ^k} > 0$ while complete shakedown takes place as ${\mu ^k} = 0$ since the rule reduces to the original Ohno-Wang multi-linear rule defined by Equation (\ref{Eqn:Introduction2}).
...
\begin{equation}
{{\dot{\bf a}}^k} = {c^k}{r^k}\left[ {{\bf{n}} - {{\left( {\frac{{\left\| {{{\bf{a}}^k}} \right\|}}{{{r^k}}}} \right)}^{{\chi ^k} + 1}}{{\bf{L}}^k}} \right]\dot p,
\end{equation}
where ${{\bf{L}}^k} = {{{\bf{a}}^k}}/{\left\| {{{\bf{a}}^k}} \right\|}$.
...
Cyclic hardening can be considered with $c^k$ in the hardening relation Equation (\ref{Eqn:Introduction3}) being a function of the accumulated plastic strain,
\begin{equation}
{c^k} = {c_{i0}}\left( {1 + {a_{i1}}{e^{ - {b_{i1}}p}} + {a_{i2}}{e^{ - {b_{i2}}p}}} \right),
...
\begin{equation}
{{\dot{\bf a}}^k} = {\zeta ^k}\left( {\frac{2}{3}{r^k}{{{\bm{\dot \upepsilon }}}^{\rm{p}}} - \left[ {{\mu ^k} + H\left( {{f^k}} \right)\left( {1 - {\mu ^k}} \right)} \right]\dot p{{\bf{a}}^k}} \right).
\end{equation}
It is clear that, as $\mu^k=0$ this rule reduces to the modified Ohno-Wang rule defined by Equation (\ref{Eqn:Introduction2}) while it reduces to the Armstrong-Frederick rule when $\mu^k>0$.
...
Following suggestions by D\"oring et al. \cite{Doerring2003} and Abdel Karim et al. \cite{AbdelKarim20051303}, Fang \cite{fang2015cyclic} suggested that the evolution equation of the backstress can be expressed as
\begin{equation}
\label{Equ:dotak1}
...
\noindent
The characteristic stress-strain curve for cyclic loading is obtained from material testing subjected to alternating loads. Observations on stress-strain hysteresis loops of the nickel-based super alloy showed that the peak stresses were not monotonic to plastic strain \cite{fang2015cyclic}. The alloy demonstrated initial cyclic hardening behavior and then cyclic softening. To describe such complex behavior $r^k$ should consist of two parts, $r_0^k$ and $r_{\Delta}^k$ \cite{fang2015cyclic}, that is,
\begin{equation}
\label{Equ:rk1}
...
Generally if the material performs Masing behavior, we can choose ${\gamma _{\rm q}}=0$.

In this study, the temperature rates of all performed tests were lower than 3.89$^\circ$C/s, and the derivative item of temperature is neglected.
