...
热电偶的位置分布如\ref{Fig:OneLightRadiation}(b)所示，内壁和外壁在垂直方向各五个。
热电偶的数据采集，选取National Instruments公司的9213型16通道热电偶数据采集器（见\ref{Fig:OneLightRadiation}(c)），该数据采集器内置CJC冷端温度补偿，通过在PC端安装NI-DAQmx驱动软件实现与电脑之间的通信。 National Instruments同时提供了LabVIEW SignalExpress软件，用于实现数据在PC端的处理和存储。
根据Equation \ref{Equ:Inverse_polynomials}，通过电脑程序将电压信号转换为温度信号，并按照顺序写入对应的文件，程序框图如\ref{Fig:OneLightRadiation}(e)所示。

为了保证实验的精度，在圆柱外表面的中心位置，我们额外增加了一个红外温度测量点。
红外测温仪为Voltcraft IR 2200型（见\ref{Fig:OneLightRadiation}(d)），测量温度范围-50至2000$^{\circ}$C。
...
\begin{figure}
  ...
  \begin{minipage}[t]{0.5\linewidth}
    ...
    \centerline{(b) Thermocouple measurement points.}
...
将所有热电偶与数据采集器连接，标定每个热电偶的温度值并进行冷端补偿。
